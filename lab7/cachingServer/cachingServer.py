...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str):
        ...
        item = self.cacheTable.get(path)
        if not item or self.cacheTable.expired(path):
            re = self.requestMainServer(path)
            if not re:
                yield (None, None)
                raise StopIteration
            hd = re.getheaders()
            self.cacheTable.setHeaders(path, hd)
            yield (hd, "😨")  # use '😨' to avoid error
            buf = bytearray(BUFFER_SIZE)
            while True:
                length = re.readinto(buf)
                if length:
                    self.cacheTable.appendBody(path, buf[:length])
                    yield (None, buf[:length])
                else:
                    raise StopIteration
        else:
            yield (item.headers, item.body, 1)
            raise StopIteration
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    def do_do_HEAD(self):
        ...
        trace个啥啊害我过不了test😡
        '''            
        resp = self.server.touchItem(self.path)
        what = next(resp)
        if type(what[-1]) == int:
            self.sendHeaders(what[0])
            yield (None, what[1])
            raise StopIteration
        else:
